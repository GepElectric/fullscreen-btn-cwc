...
echo Building FullscreenModule EXE...
echo Working directory: "%ROOT%"

python -m PyInstaller --noconfirm --clean --windowed --onedir --name FullscreenModule --paths "%ROOT%" --paths "%ROOT%\fullscreen_module" "%ROOT%\main.py"
...
set STAGE_DIR=%ROOT%\dist\fullscreen_module_package
set ZIP_PATH=%ROOT%\dist\FullscreenModule.zip
if exist "%STAGE_DIR%" rmdir /s /q "%STAGE_DIR%"
mkdir "%STAGE_DIR%"
if exist "%ZIP_PATH%" del /q "%ZIP_PATH%"

xcopy "%ROOT%\dist\FullscreenModule" "%STAGE_DIR%\FullscreenModule" /e /i /y >nul
